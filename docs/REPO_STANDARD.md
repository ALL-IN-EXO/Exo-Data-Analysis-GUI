...
│   └── ...                  # 按需: DATA_FORMAT, PLOTTING_GUIDE, API_GUIDE 等
│
├── scripts/                 # 自动化脚本
...
│
├── data/ 或 data_output/    # 数据目录（大文件考虑 .gitignore）
│   ├── sample_data/
│   └── output/
│
└── tests/                   # 测试（如适用）
...
- **不要在根目录散落源代码文件**。所有 `.py` / `.c` / `.cpp` 入口文件最多一个 `main` 文件在根目录，其余全部放 `src/`
- **不要在根目录散落文档**。除 `README.md` 和 `CLAUDE.md` 外，所有文档放 `docs/`
- **不要把数据文件提交到 Git**（除 sample_data）。大文件用 `.gitignore` 排除
...
| 类型 | 格式 | 示例 |
|------|------|------|
| 新功能 | `feature/<name>` | `feature/explorer-tab` |
| Bug 修复 | `fix/<name>` | `fix/csv-encoding-crash` |
| 文档 | `docs/<name>` | `docs/update-data-format` |
| 重构 | `refactor/<name>` | `refactor/move-utils` |
...
- **本地**：`git branch -d <branch>`，或用 `cleanup_branch.sh`
...
| 场景 | 版本变化 | 示例 |
|------|----------|------|
| 新功能 / 大改动 | Y + 1 | v1.0 → v1.1 |
| 重大架构变更 / 不兼容 | X + 1 | v1.3 → v2.0 |
...
- [ ] 专项文档（DATA_FORMAT / PLOTTING_GUIDE / API_GUIDE）— 如果改了相关内容

---

## 6. 脚本规范

所有自动化脚本放 `scripts/`，必须满足：
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v0.1 | 2026-03-20 | 初版。基于 Exo-Data-Analysis-GUI 项目经验总结 |
...
- [ ] CI/CD 基础模板（GitHub Actions: lint + test）
- [ ] 硬件/固件项目的结构适配（PCB、嵌入式）
- [ ] 多人 code review 流程细化
