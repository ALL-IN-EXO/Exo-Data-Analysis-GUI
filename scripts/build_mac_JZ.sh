...
#
# Usage:
#   ./scripts/build_mac_JZ.sh
#
# Optional env vars:
#   PYTHON_BIN=/usr/local/bin/python3
#   APP_NAME=HipExoDataAnalyzer
#   APP_VERSION=1.2.0
...
APP_NAME="${APP_NAME:-HipExoDataAnalyzer}"
...
extract_app_version() {
  local v
  v="$(grep -Eo 'Current Version: v[0-9]+([.][0-9]+)+' "${README_FILE}" 2>/dev/null | head -n1 | sed -E 's/.*v//' || true)"
  if [[ -z "${v}" ]]; then
